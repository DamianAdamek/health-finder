...
export class SchedulingController {
    ...
    @ApiResponse({ status: 201, description: 'Harmonogram utworzony', type: ScheduleDto })
    ...
    @ApiResponse({ status: 200, description: 'Lista harmonogramów', type: [ScheduleDto] })
    ...
    @ApiResponse({ status: 200, description: 'Harmonogram', type: ScheduleDto })
    ...
    @ApiResponse({ status: 200, description: 'Harmonogram zaktualizowany', type: ScheduleDto })
    ...
    @ApiResponse({ status: 201, description: 'Okno utworzone', type: WindowDto })
    async createWindow(@Body() createWindowDto: CreateWindowDto): Promise<WindowDto> {
        return this.schedulingService.createWindow(createWindowDto);
    }

    @Get('window')
    @ApiOperation({ summary: 'Pobierz wszystkie okna' })
    @ApiResponse({ status: 200, description: 'Lista okien', type: [WindowDto] })
    async getAllWindows(): Promise<WindowDto[]> {
        return this.schedulingService.getAllWindows();
    }

    @Get('window/:id')
    @ApiOperation({ summary: 'Pobierz okno po ID' })
    @ApiResponse({ status: 200, description: 'Okno', type: WindowDto })
    async getWindowById(@Param('id') id: string): Promise<WindowDto> {
    ...
    @ApiResponse({ status: 200, description: 'Lista okien', type: [WindowDto] })
    async getWindowsByScheduleId(@Param('scheduleId') scheduleId: string): Promise<WindowDto[]> {
        return this.schedulingService.getWindowsByScheduleId(+scheduleId);
    }

    @Patch('window/:id')
    @ApiOperation({ summary: 'Aktualizuj okno' })
    @ApiResponse({ status: 200, description: 'Okno zaktualizowane', type: WindowDto })
    async updateWindow(
        @Param('id') id: string,
    ...
    @ApiResponse({ status: 201, description: 'Trening utworzony', type: TrainingDto })
    async createTraining(@Body() createTrainingDto: CreateTrainingDto): Promise<TrainingDto> {
        return this.schedulingService.createTraining(createTrainingDto);
    }

    @Get('training')
    @ApiOperation({ summary: 'Pobierz wszystkie treningi' })
    @ApiResponse({ status: 200, description: 'Lista treningów', type: [TrainingDto] })
    ...
    @ApiResponse({ status: 200, description: 'Trening', type: TrainingDto })
    async getTrainingById(@Param('id') id: string): Promise<TrainingDto> {
        return this.schedulingService.getTrainingById(+id);
    }

    @Patch('training/:id')
    @ApiOperation({ summary: 'Aktualizuj trening' })
    @ApiResponse({ status: 200, description: 'Trening zaktualizowany', type: TrainingDto })
